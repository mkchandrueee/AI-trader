...
# Live AngelOne expiry list (cached for 1 hour to avoid re-parsing the
# ~150k-row instrument master on every call)
...
def _fetch_live_expiries_from_angelone() -> list[date]:
    """
    Pull the authoritative upcoming-expiry list for NIFTY options from
    AngelOne's instrument master (free, no-auth, republished daily).
    ...
    eventually break. The instrument master directly lists which contracts
    are currently listed, so there's no ambiguity about what's live.
    ...
    try:
        from data.angelone_symbols import resolver

        raw = resolver.expiries_for("NIFTY", instrument_type="OPTIDX")
        out: list[date] = []
        for ln in raw:
            try:
                out.append(datetime.strptime(ln, "%d%b%Y").date())
            except ValueError:
                continue
        out.sort()
        _LIVE_EXPIRIES = out
        _LIVE_EXPIRIES_FETCHED_AT = now
        logger.info(f"Fetched {len(out)} live NIFTY expiries from AngelOne instrument master: next 5 = {out[:5]}")
        return out
...
def get_nearest_expiry(ref_date: date) -> Optional[date]:
    ...
    Two distinct paths:
      ...
      • **Today or future dates**: use AngelOne's instrument master as the
        source of truth. NSE periodically moves NIFTY weekly expiry day
        (Thu → Wed → Tue → Mon historically), and individual weeks may be
        holiday-shifted, so any hard-coded weekday eventually breaks.
    ...
    `data.angelone_symbols.resolver.expiries_for("BANKNIFTY")` for those.
    """
    today = date.today()
    ...
    live = _fetch_live_expiries_from_angelone()
    for e in live:
        if e >= ref_date:
            return e

    # AngelOne unreachable — fall back to DB (best effort), but still
...
def load_option_premiums_for_day(symbol: str, trading_date: date) -> pd.DataFrame:
    ...
    # Below that, the candles probably have richer info (AngelOne backfills
