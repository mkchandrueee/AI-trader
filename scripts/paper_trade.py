...
Paper Trading Mode
──────────────────
Connects to AngelOne's real-time WebSocket and runs the full trading pipeline
...
def on_tick_paper(tick: dict, process_fn):
    ...
    symbol_map = {v: k for k, v in ANGEL_INDEX_SPOT_SYMBOLS.items()}
    if symbol in symbol_map:
        symbol = ANGEL_INDEX_FUTURES_SYMBOLS.get(symbol_map[symbol], symbol)
        tick["symbol"] = symbol
...
def run_live_paper():
    ...
    if not td.authenticate():
        ...
        return
    print(f"  AngelOne:        authenticated")
    ...
    # Subscribe to NIFTY
    subscribe_symbols = [ANGEL_INDEX_FUTURES_SYMBOLS.get("NIFTY", "NIFTY-I")]
