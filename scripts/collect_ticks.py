...
─────────────────────────────
Connects to AngelOne WebSocket at market open and collects tick data
...
from config.settings import (
    SYMBOLS, ANGEL_INDEX_FUTURES_SYMBOLS, ANGEL_INDEX_SPOT_SYMBOLS,
...
def on_tick(tick: dict):
    ...
    if symbol in ANGEL_INDEX_SPOT_SYMBOLS.values():
        return
...
def main():
    ...
    if not td.authenticate():
        logger.error("AngelOne authentication failed. Check credentials.")
        return
    ...
    for sym_key in SYMBOLS:
        futures_sym = ANGEL_INDEX_FUTURES_SYMBOLS.get(sym_key)
        if futures_sym:
    ...
    while running and datetime.now() < close_time:
        ...
        # The _stream_loop in market_data_adapter handles auto-reconnect on exceptions.
