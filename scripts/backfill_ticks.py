"""
Backfill missing/sparse tick data.

DORMANT since the TrueData → AngelOne/jugaad-data migration: AngelOne's free
API has no historical tick-level endpoint at all (not even a short rolling
window), so `MarketDataAdapter.fetch_historical_ticks` always returns empty
(see the documented gap in data/market_data_adapter.py). This script's gap
*detection* logic (get_tick_counts, find_sparse_days) still runs, but the
fetch step below will never actually fill anything. Kept for when/if a tick
data source is added, and because the sparse-day report is still useful
diagnostically. Use scripts/backfill_today.py / fetch_missing_ticks.py for
candle backfill, which does work.
...
Usage:
    ...
    python scripts/backfill_ticks.py --symbols NIFTY          # specific symbol only
...
# Historical retention window this script checks against (dormant — see module docstring)
TICK_HISTORY_WINDOW_DAYS = 5
...
def identify_gaps(
    ...
    AND within the TICK_HISTORY_WINDOW_DAYS window (dormant — see module docstring).
    """
...
def backfill_day(td: MarketDataAdapter, target_date: date, symbol: str) -> int:
...
def main():
    parser = argparse.ArgumentParser(description="Backfill missing tick data (dormant, see module docstring)")
    parser.add_argument(
        "--from-date", default="2026-03-10",
        help="Check coverage from this date (YYYY-MM-DD, default: 2026-03-10)"
    )
    parser.add_argument(
        "--symbols", nargs="+", default=["NIFTY"],
        help="Symbols to check (default: NIFTY)"
    )
    parser.add_argument(
    ...
    today = date.today()
    window_start = today - timedelta(days=TICK_HISTORY_WINDOW_DAYS)
    ...
    logger.info(f"Tick history window: {window_start} → {today}")
    logger.info("")
    ...
    if not gaps:
        logger.info("No sparse days found within the tick history window. All good!")
        return 0
